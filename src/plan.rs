...
impl BackupPlan{
    ...
    pub fn from_saved(plan: BackupPlanSave) -> BackupPlan{
        ...
        for destination in plan.destinations{
            destinations.push(Box::new(FileSystemDestination::new(destination)) as Box<dyn Destination>);
        }
    ...
    fn process_batch(&self, batch_name: String) {
        ...
        info!("Wrote {} files in {:.2}s", writen, batch_start_time.elapsed().as_secs_f64());
    }
    // 1. Load Batch
